...
def makeDep2FileName(n):
    return str('dep2ex' + str(n) + '.csv')
...
#    d = getYearTwoFirstGreen(getData(makeDep2FileName(n)))
#    print(n, d)

# table 6 gdp
for n in range(61, 101):
    fname = makeDep2FileName(n)
    dnum = getYearTwoFirstGreen(getData(fname))
    r = map(getOnlyNeededKeys, getCsvDicts(fname))
    y1 = getYearOneData(r)
    y2 = getYearTwoData(r)
    y1s = getPricesOrSupplies(y1, 's')
    y1p = getPricesOrSupplies(y1, 'p')
    ...
    gdptyppi = turnToPct(gdptyp1, gdptyp2)
    gdplyppi = turnToPct(gdplyp1, gdplyp2)
    print n, dnum, (gdptyppi + gdplyppi) / 2
