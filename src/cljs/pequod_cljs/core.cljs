...
(defn show-sum-of-exponents [wc]
  (->> wc
       ((juxt :input-exponents :nature-exponents :pollutant-exponents :labor-exponents))
       flatten
       (apply +)))
...
(defn check-surpluses [t]
  (letfn [(check-producers [surpluses producers inputs]
            (map #(> (Math/abs (nth surpluses (dec %)))
                      (* (:surplus-threshold t)
    ...
    (let [surplus-threshold (:surplus-threshold t)
          ...
          intermediate-good-producers (get-producers (:wcs t) 1)
          private-goods-check (check-producers (:private-good-surpluses t) private-good-producers (:private-goods t))
          im-goods-check (check-producers (:intermediate-good-surpluses t) intermediate-good-producers (:intermediate-inputs t))
...
(defn iterate-multiple-times-button []
  [:input {:type "button" :value "Iterate 10X"
           :on-click #(swap! globals proceed-iterate-n globals)}])
